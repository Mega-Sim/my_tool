...
* @param[in] secondary   = if >0 then use secondary stack instead of primary, 주 포트인지 중복 포트인지 결정하게 하는 플래그
* @return >0 if succeeded
*/
int ecx_setupnic(ecx_portt *port, const char *ifname, int secondary)    // 이더켓 포트 설정
{
   int i;
   ...
   usec_per_tick =  USECS_PER_SEC / sysClkRateGet();  // sysClkRateGet()함수를 사용하여 시스템 클록주기를 얻고 마이크로초당 틱수를 계산
    ...
    */
   if (secondary) // 중복처리 'secondary' 플래그
   {
